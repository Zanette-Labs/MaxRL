...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    # Only keep key metrics in val-core: mean@N and best@N/mean (pass@N)
                    # All others go to val-aux
                    if var_name == core_var and (f"@{n_max}" in metric_name):
                        if metric_name == f"mean@{n_max}" or metric_name == f"best@{n_max}/mean":
                            metric_sec = "val-core"
                        else:
                            metric_sec = "val-aux"
                    else:
        ...
        # -------- majority vote accuracy final metrics (in val-core, no global) --------
        n_val = self.config.actor_rollout_ref.rollout.val_kwargs.n
        for ds, tot in per_ds_mv_total.items():
            if tot > 0:
                metric_dict[f"val-core/{ds}/acc/majority@{n_val}"] = per_ds_mv_correct[ds] / tot

        # Aggregate math500_level1-5 majority voting into math500_all
        # Only do this if math500_all was not already computed by reward manager
        if found_levels and "math500_all" not in per_ds_mv_total:
            math500_mv_correct = sum(per_ds_mv_correct.get(lvl, 0) for lvl in found_levels)
            math500_mv_total = sum(per_ds_mv_total.get(lvl, 0) for lvl in found_levels)
            if math500_mv_total > 0:
                metric_dict[f"val-core/math500_all/acc/majority@{n_val}"] = math500_mv_correct / math500_mv_total
    ...
    def fit(self):
        ...
        logging_config = OmegaConf.to_container(self.config, resolve=True)
