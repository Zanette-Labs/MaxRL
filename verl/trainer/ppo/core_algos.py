...
from verl.trainer.ppo.maclaurin import maclaurin_weights, maclaurin_weights_loo, vr_cond_weights, maclaurin_weights_cross_fitted, oversample_subset_vr_weights
import numpy as np
import torch
...
def compute_p_normalization_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            #     scores[i] = ((scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)) / max(id2mean[index[i]], 0.2)

            # else:
            #     scores[i] = (scores[i] - id2mean[index[i]]) / (len(id2score[idx]) * id2mean[index[i]] + epsilon)
